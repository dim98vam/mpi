...
knnresult distrAllkNN(double* X,int n,int d,int k){
   ...
   if(pid!=0){
    ...
    for(i=0;i<nproc-1;++i){
      ...
      /*processes with odd ids send data then receive while processes with
        even ids to the opposite in order to avoid deadlocks during synchronous communication*/

      if(pid%2!=0){
       //send info forward
       MPI_Send(X,n*d,MPI_DOUBLE,(pid==nproc-1)?0:(pid+1),tag1,MPI_COMM_WORLD);
       MPI_Send(indexes,n,MPI_INT,(pid==nproc-1)?0:(pid+1),tag2,MPI_COMM_WORLD);

       //receive points for next calculation
       MPI_Recv(XtoCome, n*d, MPI_DOUBLE,(pid==0)?nproc-1:(pid-1), tag1, MPI_COMM_WORLD,&status[0]);
       MPI_Recv(indexesToCome, n, MPI_INT, (pid==0)?nproc-1:(pid-1), tag2, MPI_COMM_WORLD,&status[1]);
      }
      else{
        //receive points for next calculation
       MPI_Recv(XtoCome, n*d, MPI_DOUBLE,(pid==0)?nproc-1:(pid-1), tag1, MPI_COMM_WORLD,&status[0]);
